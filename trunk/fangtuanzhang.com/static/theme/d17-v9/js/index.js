    ...
    doajax: function(value) {
        ...
        var s = $.ajax({
            ...
            success: function(data) {
                ...
                data = eval('(' + data + ')');
                if (data.error != 0) {
                    errmsg = errmsg == "" ? data.data: errmsg; (type != 'checkbox' && type != 'radio' && element.addClass(validator.errorInput));
...
validator.showErr = function(element, errindex) {
    ...
    var msgid = jQuery('#' + element.attr('msgid'));
    var type = element.attr('type'); (type != 'checkbox' && type != 'radio' && element.addClass(this['errorinput']));
    if (!errmsg) return false;
    if (msgid.length > 0) {
...
window.x_init_hook_team = function() {
    $('#deal-buy-quantity-input').bind("keyup", 
    function() {
...
X.misc.multiclock = function(timeleft, counter) {
    ...
    var e = function() {
        ...
        if (ls > 0) {
            ...
            var ls = parseInt(Math.round(ls % 60000) / 1000);
            if (ld > 0) {
                var html = '<span><strong>' + ld + '</strong>天</span><span><strong>' + lh + '</strong>时</span><span><strong>' + lm + '</strong>分</span><span><strong>' + ls + '</strong>秒</span>'
            } else {
                var html = '<span><strong>' + lh + '</strong>时</span><span><strong>' + lm + '</strong>分</span><span><strong>' + ls + '</strong>秒</span>'
            }
            jQuery("ul#" + counter).html(html)
        } else {
            jQuery("ul#" + counter).stopTime('"+counter');
