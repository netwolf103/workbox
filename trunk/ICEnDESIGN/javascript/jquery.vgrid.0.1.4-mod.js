...
(function(jQuery)
{
	function makePos(self, initpos) //mod (added initpos argument)
	{
		...
		/* ----- mod start ----- */
		var _bodyW = jQuery(window).width();
		var _defW = self.data("_vgopt").gridDefWidth || 150 + 15 + 15 + 1;
		var _selRefGrid = self.data("_vgopt").selRefGrid || "#grid-wrapper div.x1";
		var _selFitWidth = self.data("_vgopt").selFitWidth || "#container";
		
		if ( jQuery(_selRefGrid).get().length ) 
		{
			var _colW = getSize(jQuery(_selRefGrid).eq(0))[0]; //use _w only
		}
		else 
		{
			var _colW = _defW;
		...
		if ( jQuery.isArray(_selFitWidth) ) 
		{
			for (var i=0; i<_selFitWidth.length; i++)
			{
				jQuery(_selFitWidth[i]).width( _colWSum );
			}
		}
		else
		{
			jQuery(_selFitWidth).width( _colWSum );
		}
